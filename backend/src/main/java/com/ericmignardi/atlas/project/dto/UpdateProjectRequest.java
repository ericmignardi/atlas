...
/**
 * Every field is a {@link JsonNullable} so the service can tell three intentions
 * apart: key absent means leave the field alone, key present with a value means
 * set it, key present and null means clear it. A plain {@code String client}
 * collapses the first and third into the same Java null, and the field is then
 * wiped on every partial update — silently.
 *
