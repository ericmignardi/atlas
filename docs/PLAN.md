...
| 6 | Mon Sep 7 | Frontend foundation | Done | Tokens, API client, Zod schemas, three stores, the route table with both guards, 19 primitives, the shell, and the two auth pages — 17 new frontend tests, and 143 kB gzipped against the 350 kB of NFR-1.3. Departures from §6.1 – §6.8. **Six namespaces are reset, not three**: `--font-*`, `--radius-*`, `--shadow-*`, `--ease-*`, and `--animate-*` join `--breakpoint-*`, `--color-*`, and `--text-*`, on the same reasoning — a stock `shadow-lg` is as far outside the system as a stock blue. Three colours are re-added *after* the reset (`transparent`, `current`, `inherit`): they are how CSS says "no colour" and "the inherited one", and deleting them breaks `border-transparent` and `fill-current` while enforcing nothing. So is `--color-on-accent`, because "white text on a filled button" should be one named decision rather than a hex in four variant maps. The §9.5 tag recipes became **21 `--color-tint-*` tokens shared by tags and status badges** — the two are the same three colours in different shapes, and giving each its own copy guarantees they drift. **`bg-blue-500` does not "fail to compile"**: Tailwind v4 silently emits nothing for a candidate it cannot resolve, so the check as written is not observable. Verified in the observable form instead — built with `bg-blue-500` in a component, grepped `dist`, no rule generated, while `.text-lg` resolved to the 17 px token. **`react-router` is v8, not the v7 of §6.5** — v8.3.1 is what Day 1 installed; the data-router API in the table is unchanged. Two components §6 does not list, both load-bearing. `AuthGate` resolves a third auth state the plan's two do not cover: on a cold load a persisted refresh token is neither valid nor invalid until it is redeemed, and without a state for that, `ProtectedRoute` reads the gap as "anonymous" and a refresh signs the user out of their own session. `WidthNotice` implements §8.5 as `md:hidden`, which works precisely *because* there is no breakpoint below `md`. The auth pages sit outside it — signing in works fine at 600 px even though the board does not. `restoreSession` routes the cold-load refresh through the *same* shared promise as the interceptor, so a boot racing a first request still produces one refresh, not two. NotFound renders inside the shell **and behind the guard**: an anonymous visitor to a bad URL should be asked to sign in, because the answer may be different once they are. Store actions are `signIn`/`signOut`, not §6.4's `login`/`logout` — those names belong to the two functions in `authApi.ts` that actually call the server. Feature pages are honest placeholders until Days 7–9. **The browser checks were converted to tests**, the Chrome extension not being connected: the refresh guard (five parallel 401s → one refresh, four replays), FR-7.7 (`⌘\` fires outside a text input, not inside, Escape always), both guards including the attempted-path round trip, and FR-8.4 against a 400 body copied from a live response. Registration, the proxy, and the `fields` shape were verified with curl through the dev server against the running backend. |
| 7 | Tue Sep 8 | Projects + Tags UI | Done | `useApi`, `useDebouncedValue`, `projectsApi`, `tagsApi`, `projectFilters`, `dates`, `Tabs`, `Menu`, `ProjectCard`, `ProjectToolbar`, `ProjectFormModal`, `TagInput`, `StackInput`, `TagFormModal`, and the three real pages — 24 new frontend tests, 41 green, 112 kB gzipped on first load. Departures from §7.1 – §7.5. **Hand-rolled hooks, not TanStack Query**, and the decision is written into `useApi.ts` rather than left implicit: at this scale a cache is a second source of truth to invalidate at every call site. What the hook does buy is the guard a per-page `useEffect` would not have — each run carries a sequence number, so the slow response to "at" cannot overwrite the fast one to "atl". **The card shows one environment count, not chips by type.** `ProjectResponse` carries only a total; the per-type split needs `GET /environments?projectId=`, and fanning that out per card is one request per project to render a list. The split lands on the detail page tomorrow, where the data is loaded anyway. **`api.delete` became generic in its result**, because `DELETE /projects/{id}/pin` returns the updated project rather than 204 — verified against a live server, along with the 200-not-409 on a duplicate tag name, the slug regenerating on rename, and `tagIds: []` clearing the set. **A third empty state exists** that §9.7 does not name: an account whose every project is archived has rows, no filters, and nothing on screen — neither "no projects yet" nor "no matches" answers that, so it gets its own state offering *Include archived*. **`Tabs` attaches its parts to a function declaration rather than `Object.assign`**: the latter is a call expression, and a module whose only export is one reads to Fast Refresh as a module exporting a non-component. **Two bugs in Day 6 code, both found by building on it.** `useFocusTrap` depended on `onEscape`, which every caller passes as an inline arrow — so the effect re-ran on every render, and re-running it calls `.focus()`. Inside a confirmation dialog that is invisible; inside a dialog holding a form, the caret jumps to the close button after the first character typed. The symptom is "the form only accepts one letter", and it is nowhere near the input. The handler now lives in a ref and the effect depends on `[container, active]`. Second: the trap focused the first focusable element in DOM order, which is the header's dismiss button, so every form opened on Cancel — it now prefers `[data-autofocus]`, and `autoFocus` is not used for this because React implements it by calling `.focus()` rather than by emitting the attribute. `Modal.test.tsx` covers both. **The browser checks were again converted to tests**, the Chrome extension being unconnected: the pin rollback (optimistic toggle, 409, icon back), the two empty states and the Clear-filters round trip, ⌘Enter creating a project that appears without a refetch, the delete confirmation naming the project and its environment count, and `?tab=` in both directions. The endpoint contracts behind all of it were verified with curl against the running backend. |
| 8 | Wed Sep 9 | Environments + Tasks UI | Done | `environmentsApi`, `tasksApi`, `environmentFormat`, `environmentPairing`, `taskOrder`, `taskFilters`, `EnvironmentTile`, `EnvironmentGroupCard`, `PairDialog`, `EnvironmentFormModal`, `EnvironmentMap`, `TaskCard`, `TaskBoard`, `TaskList`, `TaskToolbar`, `TaskFormModal`, `TaskWorkspace`, the two real pages and both project tabs — 32 new frontend tests, 73 green, 196 kB gzipped across every chunk against the 350 kB of NFR-1.3 (119 kB on first load). Departures from §8.1 – §8.4. **`/grouped` cannot be rendered directly** as §8.1 says: it answers in `EnvironmentSummary` — id, name, platform, branch — and a tile also shows the URL, the notes indicator, and a menu that needs the type. So `/environments?projectId=` is fetched in parallel and each summary resolved by id: shape from the server, detail from the record, and the pairing arithmetic still written down once. **The midpoint in §8.2 runs out immediately.** `sortOrder` is an `int` and every task is created at `min - 1`, so a fresh column is …-2, -1, 0 with no gaps at all and *every* drop between two adjacent cards has no integer to take — a naive midpoint writes a duplicate, the two cards tie, the database breaks the tie arbitrarily, and the board comes back from a reload in a different order than the one just arranged. A move is therefore a **plan**: one PUT in the ordinary case, and a compaction of the destination column onto a stride of 64 when the neighbours are adjacent, which also heals an existing tie. Verified against the running server — the three PUTs a compaction sends land as 0/64/128, and the next drop into the same gap is a single PUT at 32. **Drop zones, not pointer arithmetic.** The zones between cards carry their own index and *are* the 8 px the column wanted as `gap-2`, so nothing shifts when a drag begins; midpoint-of-rectangle maths re-measures on every `dragover` frame and is untestable in jsdom, which reports every rectangle as zero. The arithmetic worth testing is in `taskOrder.ts` as a pure function instead. **The board is not filtered** and that is a limit, not an omission: a board *is* the status axis, and a compaction renumbers only the cards it can see, so any filter would silently reorder the hidden ones. It is scoped by project, which the server does; §8.3's filters are list-only. **The pair dialog has a fourth rule the server does not**, beyond §8.1's same-project / same-type / not-itself: the candidate must be the opposite kind, because FR-3.15 draws a row as `app ── database` and two paired applications produce a row that has no shape. A candidate that is already paired *is* still offered, with the displacement stated on the row, because FR-3.11 releases rather than refuses. **One backend fix, found by building the UI on it.** `GET /environments?projectId=` reported `pairedWith` from the `paired_with_id` column alone while `GET /environments/{id}` and `/grouped` both resolve the reverse direction — so against the real dev database every seeded Neon row read as unpaired while its application read as paired, and the tile chooses between "Pair…" and "Unpair" on exactly that field. `EnvironmentService.list` now builds the same claimant map `grouped` does and narrows by type and platform *after* it, because a `platform=NEON` filter would otherwise drop the very row that points at the row being returned. One query either way; `EnvironmentControllerIT` gains a test for both ends, 188 green. **A due date is stored at the end of its day, not the start**: a date input speaks `yyyy-MM-dd`, the column is an `Instant`, and FR-4.9 compares it against `now` — stamped at midnight a task due today is overdue by breakfast, stamped at 23:59:59.999 local it stays in FR-4.10's *today* bucket until the day actually ends. **The status select is a move, the list checkbox is a PATCH.** NFR-4.6 says the select does what a drag does, so it goes through the identical `planMove(destination, task, status, 0)` and `PUT /move`; the list shows no positions to compute one from, so it takes the server's own top-of-column rule. **The browser checks were again converted to tests**, the Chrome extension still not connecting: group order with the dashed slot and the orphan, the eligible-partner filter, pairing updating both tiles without a refetch, both consequences of a type change, a drag between columns and a drop between two cards with the exact PUT bodies, the select producing the byte-identical request, the rollback and its toast, and the Done window. Every contract behind them was verified with curl against the running backend: `/grouped`'s three groups, the now-symmetric list, `PAIR_SELF`, `PUT`/`DELETE /pair` returning both sides, FR-3.12 releasing both, FR-4.7's `min - 1`, FR-4.6 stamping on the way in and clearing on the way out, and the compaction arithmetic. |
